...
int direntv6_opendir(const struct unix_filesystem *u, uint16_t inr,
    ...
    struct inode inode;
    int error = inode_read(u, inr, &inode);//correcteur : not useful, you can fv6open and check the inode is a file directly after that
    if (error < 0) {
        return error;
    }
    ...
    error = filev6_open(u, inr, &d->fv6);//correcteur : you should open on a new fv6 
    d->cur = 0;
    d->last = 0;
...
int direntv6_readdir(struct directory_reader *d, char *name,
    ...
    if (d->cur == d->last) {
        int read = filev6_readblock(&d->fv6, d->dirs);//correcteur : work on a new dirs
        if (read <= 0) return read; // an error occured
#pragma GCC diagnostic ignored "-Wsign-conversion" //correcteur : bad habit
