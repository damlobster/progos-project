...
int filev6_open(const struct unix_filesystem *u, uint16_t inr,
    ...
    fv6->u = u;
    fv6->i_number = inr;
    int error = inode_read(u, inr, &fv6->i_node);//correcteur : you should try and check this before setting fv6->u and i_number
    fv6->offset = 0;
...
int filev6_readblock(struct filev6 *fv6, void *buf) {
	...
	//correcteur : you should work on a copy of fv6->offset from here 
    ...
    int error = sector_read(fv6->u->f, (uint32_t)sector, buf); //correcteur : please comment this whole section which is unclear
