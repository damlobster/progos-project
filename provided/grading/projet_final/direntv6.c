...
int direntv6_opendir(const struct unix_filesystem *u, uint16_t inr,
    ...
    //correcteur : le commentaire que vous avez fait mon collège (changement exceptionnel de correcteur entre correction projet01/2 et projet_final) dans le projet01 fait sens, dans le cas ou vous avez une erreur il est possible que votre directory_reader soit rempli de données qu'on aurait pas voulu initialisée. Il aurait été meiux de bosser avec un fv6 statique dans le code et a la fin assigner le champ correspondant de d. 
...
int direntv6_readdir(struct directory_reader *d, char *name,
    ...
    if (d->cur == d->last) {
        int read = filev6_readblock(&d->fv6, d->dirs); //correcteur : cf commentaire dans open_readdir et commentaire dans la correction projet01.
        if (read <= 0) {
...
int direntv6_create(struct unix_filesystem *u, const char *entry, uint16_t mode) {
    ...
    if (parent_dir_inr < 0) {
        //correcteur : dirlookup peut renvoyer d'autre types d'erreurs (Code négatig) que vous ne propagez pas dans ce cas, vous devriez renvoyer ERR_BAD_PARAMETER uniquement si le code d'erreur renvoyé par dirloojup est ERR_INODE_OUTOF_RANGE. (-1.5)
