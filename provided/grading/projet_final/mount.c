...
int mountv6(const char *filename, struct unix_filesystem *u) {
    ...
    //correcteur : il aurait fallut tester les valeurs de retour des deux bm_alloc (-0.5)
    fill_ibm(u);
    fill_fbm(u);
...
void mountv6_print_superblock(const struct unix_filesystem *u) {
    ...
    if (u == NULL) {
        ...
        //correcteur : mauvais formatage du print du s_time, cf. http://moodle.epfl.ch/mod/forum/discuss.php?d=7676#p15675 
...
void fill_ibm(struct unix_filesystem *u) {
    ...
    for (uint32_t k = 0; k < u->s.s_isize; k++) {
        ...
        for (unsigned int i = 0; i < INODES_PER_SECTOR; i++) {
            //correcteur : ce "if(){}else if(){}" aurait pu être modularisé en un seul (vu que la même action est effectué dans les deux cas) (-0.25)
            if (error != 0) {
...
void fill_fbm(struct unix_filesystem *u) {
    ...
    //correcteur : c'est fonctionnel, mais pas optimal, vous parcourez déjà tout les inodes qui sont alloués dans fill_ibm, autant s'occuper de regarder leurs secteurs par la même occasion, et ainsi modulariser fill_fbm pour qu'il prenne un numéro d'inode/une inode en argument et va s'occuper de chercher les secteurs utilisés par cette inode (-1)
...
int mountv6_mkfs(const char* filename, uint16_t num_blocks, uint16_t num_inodes) {
    ...
    //correcteur : vous ne fermez jamais votre file, ni dans les cas d'erreurs, ni tout court. (-2)
    if (f == NULL) {
        return ERR_IO;
    }
