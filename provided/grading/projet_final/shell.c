...
//correcteur : vous avez réussi à corriger la quasi intégralité de vos erreurs du projet01/2 dans ce rendu ! Bravo !
...
int do_mkfs(const char** args) {
    ...
    //correcteur : Très malin de controler que l'input ne dépasse pas la valuer max de uint16 (0.5 point de bonus)
...
/**
//correcteur : a présent qu'il est implémenté, vous auriez pu mettre les doxygen correctement.
 * Unimplemented
 * @param args
 * @return
 */
...
int do_add(const char** args) {
    ...
    //correcteur : 4096 est clairement 8 * SECTOR_SIZE, pourquoi ne pas avoir utilisé les macros pour le déclarer ? (-1)
    char buf[4096];
    size_t read = 0;
