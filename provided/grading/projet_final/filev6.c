...
int filev6_readblock(struct filev6 *fv6, void *buf) {
    ...
    //correcteur : merci d'avoir ajouté les commentaires, une bonne habitude à prendre/garder.
...
int filev6_create(struct unix_filesystem *u, uint16_t mode, struct filev6 *fv6) {
    ...
    //correcteur : je suis d'accord pour votre envie d'utiliser inode_alloc, vous l'avez justifié dans le README, par contre initialiser à 0 à la main tout les champs de l'inode est une mauvaise idée, vous auriez pu déclarer satiquement une inode temporaire, que vous auriez memset à 0, changer son mode, pour ensuite l'affecter à l'inode du filev6. Ca aurait fait 5* moins de ligne de code est c'est beaucoup plus simple/clair (-0.5) 
...
int create_indirect_sector(struct unix_filesystem *u) {
    //correcteur : bien d'avoir modularisé cette partie, ça rend votre writesector un chouilla moins verbeux (0.5 point de bonus)
...
int filev6_writesector(struct unix_filesystem *u, struct filev6 *fv6, const void *buf, uint32_t len) {
    // internal method: no check of arguments validity
    //correcteur : bon job pour cette fonction !
