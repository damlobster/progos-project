...
int inode_scan_print(const struct unix_filesystem *u) {
    ...
    for (uint32_t k = u->s.s_inode_start; k < u->s.s_isize; k++) {//correcteur : you should iterate isze times not isize-inode_start !
        ...
        for (unsigned int i = 0; i < INODES_PER_SECTOR; i++) {
            ...
            if (inodes[i].i_mode & IALLOC) {
                ...
                printf("inode %3zu (%s) len %4d\n",
                        i + (k - 2) * INODES_PER_SECTOR, type, size);//correcteur : why 2 ? avoid hard values
            }
        }
    }
...
int inode_read(const struct unix_filesystem *u, uint16_t inr,
        struct inode *inode) {
    M_REQUIRE_NON_NULL(u);
    M_REQUIRE_NON_NULL(inode); //correcteur : check check validity of inr
